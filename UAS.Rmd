...
# 3. Simple Randomsplit Training Testing 80:20
```{r}
set.seed(123)
...
# testing accuracy
probs1 = predict(model1, newdata=testing, type="response")
accuracy = mean(testing$LULUS == as.numeric(probs1>0.5))
print(accuracy)

testing$LULUS <- as.factor(testing$LULUS)
probs1 <- as.factor(as.numeric(probs1 > 0.5))
confusionMatrix(testing$LULUS, probs1)
```
...
probs2 = predict(model2, newdata = testing, type = "prob")[,2]

testing$LULUS <- as.factor(testing$LULUS)
probs2 <- as.factor(as.numeric(probs2 > 0.5))
...
confusionMatrix(testing$LULUS, probs5)
  ...
  navbarPage(
    ...
    div(
      ...
      div(
        ...
        sidebarLayout(
          sidebarPanel(
            selectInput("MODEL", "Model Prediksi", choices = c("Logistic Regression", "Binary Tree","Support Vector Machine", "Random Forest", "Gradient Boosting"), selected = NULL),
